...
アプリケーションは、以下の情報を収集します：

* Watchのモーションセンサーデータ（睡眠状態の検知用）
* HealthKitからの睡眠関連データ
* アプリケーションの使用状況（アクセス日時、使用時間など）
* デバイスの基本情報（OSバージョンなど）

これらのデータは以下の目的で使用されます：

* 睡眠状態の検知：Watchのセンサーデータを使用して、ユーザーの睡眠状態を判定します
...
本アプリケーションは、HealthKitを使用して睡眠関連データにアクセスします。これらのデータは：

* ローカルデバイス上でのみ処理され、外部に送信されることはありません
* ユーザーの明示的な許可がない限りアクセスされません
* 睡眠検知機能の提供にのみ使用されます
